        ...
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <form method="POST" enctype="multipart/form-data" action="{{ route('addclassTeacher') }}">
                @csrf
                <div class="row mb-3">
                    <label for="inputText" class="col-sm-2 col-form-label">Class Teacher Full Name</label>
                    <div class="col-sm-10">
                        <input name="teacher_fullname" placeholder="Teacher Full Name" required type="text" class="form-control" value="{{ old('teacher_fullname') }}">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <table class="table">
                ...
                <tbody>
                    @foreach ($teacherClasses as $index => $teacherClass)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>{{ $teacherClass['teacher_name'] }}</td>
                            <td>{{ $teacherClass['class_name'] }}</td>
                            <td>
                                <form action="{{ route('delete.teacher', ['id' => $teacherClass['id']]) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
